describe('Enterprise PNPM Template E2E', () => {
  ...
  it('should show alert when primary button is clicked', () => {
    ...
    cy.get('@windowAlert').should(
      ...
      'Hello from Enterprise PNPM template!'
    );
  });
  ...
  it('should be responsive on mobile', () => {
    ...
    cy.get('[data-testid="primary-button"]').then(($btn1) => {
      cy.get('[data-testid="secondary-button"]').then(($btn2) => {
        expect($btn1[0].getBoundingClientRect().top).to.be.lessThan(
          $btn2[0].getBoundingClientRect().top
        );
      });
    });
  });
